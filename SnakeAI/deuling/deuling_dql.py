...
if __name__ == '__main__':
    BATCH_SIZE = 64
    FILENAME = "DDDQN_CP.weights"
    MEMORYFILENAME = "MEMORY_CP.mem"
    load = False
    n_games = 1000
    env = SnakeEnv(frameRate=15)
    agent = Agent(lr=0.001, input_shape=(6,), n_actions=5)
    memory = []
    scores = []
    Avg_scores = []
    highscore = -math.inf
    ...
    plotLearning([i for i in range(n_games)], Avg_scores, scores, "snakewithreward.png")
    plt.show()
